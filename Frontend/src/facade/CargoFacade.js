...
const URL = URLSettings.ServerUrl() + "/api/Cargo/";
...
function CargoFacade() {
  ...
  function addCargo(cargo) {
    ...
    //return (person.id === "") ? 
    fetch(URL + "add", makeOptions("POST", cargo)).then(handleHttpErrors); //:
    //fetch(URL + person.id, makeOptions("PUT", person)).then(handleHttpErrors);
  }

  function editCargo(cargo) {
    fetch(URL, makeOptions("PUT", cargo)).then(handleHttpErrors);
  }
